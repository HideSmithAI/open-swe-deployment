...
def test_detects_pr_creation_fallback_commands() -> None:
    assert is_pr_creation_fallback_command("GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh pr create --draft")
    ...
    assert is_pr_creation_fallback_command(
        "GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh api -X POST repos/langchain-ai/open-swe/pulls -f title=x"
    )
    assert is_pr_creation_fallback_command(
    ...
    assert is_pr_creation_fallback_command("GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" sh -c 'gh pr create --draft'")
    assert is_pr_creation_fallback_command(
...
def test_allows_safe_pr_commands() -> None:
    assert not is_pr_creation_fallback_command("GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh pr view 1 --json url")
...
async def test_middleware_blocks_execute_pr_creation_fallbacks() -> None:
    for command in (
        "GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh pr create --draft",
        "gh api repos/langchain-ai/open-swe/pulls -X POST -f title=x",
        "GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh api -X POST repos/langchain-ai/open-swe/pulls -f title=x",
        "curl -X POST https://api.github.com/repos/langchain-ai/open-swe/pulls -d '{}'",
    ):
...
async def test_middleware_allows_safe_pr_view() -> None:
    result = await PullRequestCreationGuardMiddleware().awrap_tool_call(
        cast(ToolCallRequest, _Request("GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh pr view 1 --json url")), _handler
    )
