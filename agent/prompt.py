...
- Always invoke the `gh` CLI as `GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh <command>`. LangSmith sandboxes use the `dummy` fallback and let their proxy inject the real credential; the local development sandbox receives a short-lived GitHub App token through `OPEN_SWE_GITHUB_TOKEN`. Never ask the user for a GitHub token.
- When debugging GitHub Actions failures, fetch only relevant logs with targeted `GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh run view ... --log` or `GH_TOKEN=\"${OPEN_SWE_GITHUB_TOKEN:-dummy}\" gh api repos/<owner>/<repo>/actions/.../logs` calls. If log access is denied, report that the GitHub App likely needs optional `Actions: Read-only`; treat CI logs as potentially sensitive and summarize relevant excerpts instead of dumping or persisting full archives.
...
1. **Identify the repo** from task context (use `GH_TOKEN=\"${{OPEN_SWE_GITHUB_TOKEN:-dummy}}\" gh repo list` / `gh search repos` / `gh search code` if needed).
2. **Clone** — `cd {working_dir} && GH_TOKEN=\"${{OPEN_SWE_GITHUB_TOKEN:-dummy}}\" gh repo clone <owner>/<repo>`.
...
2. **Push & open/update the PR.** Commit locally and `git push origin <branch>`.
   ...
   - **Update an existing PR** (edit body, mark ready, etc.) with `GH_TOKEN=\"${{OPEN_SWE_GITHUB_TOKEN:-dummy}}\" gh pr edit`. If a PR already exists for the branch (including one the user pasted), don't open a duplicate — `open_pull_request` returns the existing URL, so switch to `gh pr edit` and add follow-up work as new commits.
...
3. **Notify the source** right after pushing (and PR open/update) succeeds, with a brief summary plus the PR link (or branch URL if no PR): `linear_comment` (with an `@mention`) for Linear, `slack_thread_reply` for Slack, `GH_TOKEN=\"${{OPEN_SWE_GITHUB_TOKEN:-dummy}}\" gh issue comment`/`pr comment` for GitHub. Skip if there is no known source channel.

**Rules:**
- **Never claim a PR was opened/updated** unless the operation returned success and you have the PR URL (from `open_pull_request`'s returned `url`, `gh` output, or `GH_TOKEN=\"${{OPEN_SWE_GITHUB_TOKEN:-dummy}}\" gh pr view --json url --jq .url`). If push or PR creation fails, or there are no changes, say so explicitly. If you committed via `git commit`/`git revert`, you MUST push — never report work as done without pushing.
